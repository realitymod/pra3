...
if (diag_frameNo > PRA3_recognize_nextNearCheck) then
{
	...
	PRA3_recognize_knownSquadUnits = [];
	...
	{
		{
			...
			) then
			{
				...
				if (_playerSquad == _x call PRA3_fnc_unitGetSquad) then
				{
					PRA3_recognize_knownSquadUnits set [count PRA3_recognize_knownSquadUnits, _x];
				};
			};
...
} forEach PRA3_recognize_knownSquadUnits;
...
{
	if !(_x in PRA3_recognize_knownSquadUnits) then
	{
		_x setVariable ["PRA3_stareTime", 0];
	};
} forEach PRA3_recognize_lastKnownSquadUnits;

PRA3_recognize_lastKnownUnits = PRA3_recognize_knownUnits;
PRA3_recognize_lastKnownSquadUnits = PRA3_recognize_knownSquadUnits;
