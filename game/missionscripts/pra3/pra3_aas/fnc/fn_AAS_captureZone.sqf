...
if (isClient) then
{
	if (_prevOwner == __neutral) then
	{
		...
		[format ["PRA3_AAS_captureNotification%1",_text],[str (_zone call PRA3_fnc_AAS_getZoneName) ,((_zone call PRA3_fnc_AAS_getZoneOwner) call PRA3_fnc_sideToTeam) ,"\a3\Ui_f\data\GUI\Cfg\Notifications\tridentEnemy_ca.paa",""]] call bis_fnc_showNotification;
	}
	else
	{
		player globalChat format["%1 lost control of %2", _prevOwner, _zone call PRA3_fnc_AAS_getZoneName];
		[format ["PRA3_AAS_neutralizedNotification%1",_text],[(_prevOwner call PRA3_fnc_sideToTeam), str (_zone call PRA3_fnc_AAS_getZoneName) ,"\a3\Ui_f\data\GUI\Cfg\Notifications\tridentEnemy_ca.paa",""]] call bis_fnc_showNotification;
	};
};
