...
PRA3_fDebugConsole_populateMonitored =
{
	...
	} forEach (profileNamespace getVariable ["PRA3_debugConsole_monitoredValues", []]);
...
PRA3_fDebugConsole_addMonitored =
{
	...
	{
		var(_monitored) = profileNamespace getVariable ["PRA3_debugConsole_monitoredValues", []];
		var(_arr) = (_monitored select lbCurSel ctrlMon(IDC_DEBUGCONSOLE_MONITOR_CATEGORY)) select 1;
		_arr set [count _arr, [_name, _expression]];
		profileNamespace setVariable ["PRA3_debugConsole_monitoredValues", _monitored];
...
PRA3_fDebugConsole_showEditMonitored =
{
	...
	if (_idx > -1) then
	{
		...
		var(_entry) = ((_monitored select PRA3_debugConsole_monitoredValuesCurrent) select 1) select _idx;
...
PRA3_fDebugConsole_editMonitored =
{
	...
	{
		var(_monitored) = profileNamespace getVariable ["PRA3_debugConsole_monitoredValues", []];
		var(_arr) = (_monitored select PRA3_debugConsole_monitoredValuesCurrent) select 1;
...
PRA3_fDebugConsole_monitoredEvaluate =
{
	...
	if (count _monitored > 0) then
	{
		...
		} forEach ((_monitored select PRA3_debugConsole_monitoredValuesCurrent) select 1);
		{
...
{
	var(_entry) = ((profileNamespace getVariable ["PRA3_debugConsole_monitoredValues", []] select PRA3_debugConsole_monitoredValuesCurrent) select 1) select (_this select 1);
...
PRA3_fDebugConsole_deleteMonitor =
{
	if (lbCurSel ctrl(IDC_DEBUGCONSOLE_MAIN_MONITOR_LIST_LB) != -1) then
	{
		[parseText format['Are you sure you wish to delete the value monitor entry "%1"?',
				(((profileNamespace getVariable ["PRA3_debugConsole_monitoredValues", []] select PRA3_debugConsole_monitoredValuesCurrent) select 1) select lbCurSel ctrl(IDC_DEBUGCONSOLE_MAIN_MONITOR_LIST_LB)) select 0
			],
			{
				var(_monitored) = profileNamespace getVariable ["PRA3_debugConsole_monitoredValues", []];
				var(_entry) = _monitored select PRA3_debugConsole_monitoredValuesCurrent;
				...
				{
					[_monitored, PRA3_debugConsole_monitoredValuesCurrent] call PRA3_fDebugConsole_removeIndex;
				}
				else
				{
	...
	if (count PRA3_debugConsole_pinnedValues == 0) then
	{
		...
		0 spawn
		{
			while {!isNull(uiNamespace getVariable "PRA3_RscDebugConsole_pinnedValues")} do
			{
				// Show values
				{
					var(_value) = ((profileNamespace getVariable ["PRA3_debugConsole_monitoredValues", []] select PRA3_debugConsole_monitoredValuesCurrent) select 1) select _x;
